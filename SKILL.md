...
| **Retrieve app icon image** | No endpoint returns the app icon file. `builds/{id}/icons` returns nothing. | Extract from the Xcode project or App Store marketing URL |
| **Delete an app** | Apps cannot be deleted via API. They can only be "removed from sale" (hidden). | Remove manually: ASC -> Apps -> [App] -> Pricing and Availability -> Remove from sale |
